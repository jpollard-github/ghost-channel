...
Inspect all 12 PNGs from the desktop, tablet-landscape, and iPhone-portrait projects for clipping, overflow, unreachable controls, spacing, and diagnostics usability. The iPhone landscape project is intentionally absent because the measured installed PWA remained portrait-only. Current screenshots remain in place so a subsequent `npm run review:change` includes them. Old timestamped screenshot packets are outside `screenshots/current` and are never copied into change-review packets. Generated screenshot and review artifacts are ignored and must not be committed.

For physical measurement, run `npm run dev:lan` on a trusted network, open the printed Network URL with `/?diagnostics=1`, rotate the device, and use **Copy device profile** in ordinary browser and standalone PWA modes. In an installed PWA without a keyboard or visible address bar, press and hold the transmission status in the upper-right corner for 1.2 seconds to open diagnostics. Adapt the relevant entry in `config/review-devices.local.json`, then recapture and inspect the review screenshots.
